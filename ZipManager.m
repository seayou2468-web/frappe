...
+ (NSArray<FileItem *> *)listContentsOfZipAtPath:(NSString *)zipPath internalPath:(NSString *)internalPath {
    mz_zip_archive zip_archive;
    memset(&zip_archive, 0, sizeof(zip_archive));
    if (!mz_zip_reader_init_file(&zip_archive, [zipPath fileSystemRepresentation], 0)) return @[];

    if (!internalPath) internalPath = @"";
    if ([internalPath hasPrefix:@"/"]) internalPath = [internalPath substringFromIndex:1];
    if (internalPath.length > 0 && ![internalPath hasSuffix:@"/"]) internalPath = [internalPath stringByAppendingString:@"/"];
    ...
    for (mz_uint i = 0; i < num_files; i++) {
        mz_zip_archive_file_stat file_stat;
        if (!mz_zip_reader_file_stat(&zip_archive, i, &file_stat)) continue;
        NSString *fullFileName = [NSString stringWithUTF8String:file_stat.m_filename];
        if ([fullFileName isEqualToString:internalPath]) continue;

        if ([fullFileName hasPrefix:internalPath]) {
            NSString *relativePart = [fullFileName substringFromIndex:internalPath.length];
            NSArray *components = [relativePart componentsSeparatedByString:@"/"];
            NSString *topComponent = components.firstObject;
            if (topComponent.length == 0 && components.count > 1) topComponent = components[1];
            if (topComponent.length == 0) continue;

            if ([foundNames containsObject:topComponent]) continue;
            [foundNames addObject:topComponent];

            FileItem *item = [[FileItem alloc] init];
            item.name = topComponent;
            item.fullPath = [NSString stringWithFormat:@"%@/%@", zipPath, [internalPath stringByAppendingPathComponent:topComponent]];
            item.isDirectory = (components.count > 1 || mz_zip_reader_is_file_a_directory(&zip_archive, i));

            NSMutableDictionary *attrs = [NSMutableDictionary dictionary];
            attrs[NSFileSize] = @(file_stat.m_uncomp_size);
            attrs[NSFileModificationDate] = [NSDate dateWithTimeIntervalSince1970:file_stat.m_time];
