...
/**
 * Handle for the MCInstall client
 */
typedef struct McInstallClientHandle McInstallClientHandle;

/**
 * Handle for the modern MCInstall client
 */
typedef struct McInstallCoreDeviceClientHandle McInstallCoreDeviceClientHandle;
...
/**
 * Creates a new modern MCInstall client from a stream
 *
 * # Safety
 * Pass a valid stream handle and a pointer to a location to store the new client handle.
 */
struct IdeviceFfiError *mcinstall_core_device_client_new_from_stream(struct ReadWriteOpaque *stream,
                                                                     struct McInstallCoreDeviceClientHandle **client);

/**
 * Frees a modern MCInstall client handle
 *
 * # Safety
 * Pass a valid handle allocated by this library.
 */
void mcinstall_core_device_client_free(struct McInstallCoreDeviceClientHandle *client);

/**
 * Retrieves the list of installed configuration profiles.
 * The returned plist_t is owned by the caller and must be freed using idevice_plist_free or similar.
 *
 * # Safety
 * Pass a valid client handle and a pointer to a location to store the result plist.
 */
struct IdeviceFfiError *mcinstall_core_device_get_profile_list(struct McInstallCoreDeviceClientHandle *client,
                                                               plist_t *out_plist);
 ...
 *
 * # Safety
 * Pass a valid client handle and a valid pointer to profile data.
 */
struct IdeviceFfiError *mcinstall_core_device_install_profile(struct McInstallCoreDeviceClientHandle *client,
                                                              const uint8_t *profile_data,
                                                              uintptr_t profile_data_len);

/**
 * Removes a configuration profile
 *
 * # Safety
 * Pass a valid client handle and a valid C string for the identifier.
 */
struct IdeviceFfiError *mcinstall_core_device_remove_profile(struct McInstallCoreDeviceClientHandle *client,
...
/**
 * Connects to MCInstall service using provider
 *
 * # Safety
 * Pass a valid provider handle and a pointer to a location to store the new client handle.
 */
struct IdeviceFfiError *mcinstall_connect(struct IdeviceProviderHandle *provider,
                                          struct McInstallClientHandle **client);

/**
 * Connects to MCInstall service using RSD
 *
 * # Safety
 * Pass a valid adapter and handshake handles and a pointer to a location to store the new client handle.
 */
struct IdeviceFfiError *mcinstall_connect_rsd(struct AdapterHandle *adapter,
                                              struct RsdHandshakeHandle *rsd,
                                              struct McInstallClientHandle **client);

/**
 * Frees an MCInstall client handle
 *
 * # Safety
 * Pass a valid handle allocated by this library.
 */
void mcinstall_client_free(struct McInstallClientHandle *client);
 ...
 * Pass a valid client handle and a pointer to a location to store the result plist.
 */
struct IdeviceFfiError *mcinstall_get_profile_list(struct McInstallClientHandle *client,
                                                   plist_t *out_plist);
 ...
 * Pass a valid client handle and a valid pointer to profile data.
 */
struct IdeviceFfiError *mcinstall_install_profile(struct McInstallClientHandle *client,
                                                  const uint8_t *profile_data,
                                                  uintptr_t profile_data_len);
 ...
 * Pass a valid client handle and a valid C string for the identifier.
 */
struct IdeviceFfiError *mcinstall_remove_profile(struct McInstallClientHandle *client,
                                                 const char *identifier);
